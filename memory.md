...
- The main rail uses one continuous card-stack model: the front page falls down-left and forward, three upcoming pages stay visibly stacked behind it, wheel/touch input is bounded continuous progress, and delayed settling only aligns the stack after input stops.
- The outgoing rail card turns partially edge-on with a slight counterclockwise roll as it falls down-left, while preserving explicit forward depth separation from the incoming stack. During motion no more than three cards remain render-visible.
- The stable rail camera uses the active page's real dimensions and current viewport aspect ratio to fit the complete page on screen; short desktop windows are height-limited and narrow phones are width-limited.
